...
@section('content')
    <main class="bg-[#FFF3F3] text-[#3F3142] min-h-screen py-8">
        <div class="w-[80%] mx-auto">
            <h1 class="text-4xl font-bold mb-8">Pesanan ku</h1>
            ...
            @else
                ...
                <div class="bg-white rounded-lg shadow-md p-12 text-center">
                    ...
                    <h2 class="text-3xl font-bold text-gray-700 mb-4">Tidak Ada Pesanan</h2>
                ...
                @if ($addresses->count() > 0)
                    <div class="space-y-3">
                        @foreach ($addresses as $address)
                            <label
                                class="flex items-start gap-3 p-4 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors {{ $address->is_default ? 'border-[#3F3142] bg-[#FFF3F3]' : '' }}">
                                <input type="radio" name="address_id" value="{{ $address->id }}"
                                    data-city-id="{{ $address->city_id }}" class="mt-1 address-radio"
                                    {{ $address->is_default ? 'checked' : '' }} required
                                    onchange="loadShippingOptions(this.value)">
                                <div class="flex-1">
                                    @if ($address->is_default)
                                        <span
                                            class="inline-block px-2 py-1 bg-[#3F3142] text-white text-xs font-semibold rounded mb-2">
                                            Default
                                        </span>
                                    @endif
                                    <p class="text-gray-700 leading-relaxed">{{ $address->detail }}</p>
                                    <p class="text-xs text-gray-500 mt-2">📍 {{ number_format($address->latitude, 4) }},
                                        {{ number_format($address->longitude, 4) }}</p>
                                </div>
                            </label>
                        @endforeach
                    </div>
                @else
                    <p class="text-gray-500">Tidak ada alamat. Silakan tambahkan alamat terlebih dahulu.</p>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mt-4">
                ...
                <div id="shipping-options-container">
                    <p class="text-gray-500 text-center py-4">
                        <svg class="animate-spin h-5 w-5 mx-auto mb-2 text-[#3F3142]" xmlns="http://www.w3.org/2000/svg"
            ...
            fetch('/api/shipping-options', {
                ...
                .then(data => {
                    ...
                    `;
                        ...
                        container.innerHTML =
                            '<p class="text-red-500 text-center py-4">No shipping options available</p>';
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    container.innerHTML =
                        '<p class="text-red-500 text-center py-4">Failed to load shipping options</p>';
                });
        }
