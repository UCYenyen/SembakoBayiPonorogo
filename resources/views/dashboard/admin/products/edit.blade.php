            ...
            <div class="bg-white rounded-lg shadow-lg p-8">
                ...
                <form action="{{ route('admin.products.update', $product->id) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="discount_amount" class="block text-sm font-medium mb-2">Diskon (Rp) *</label>
                            <input type="number" name="discount_amount" id="discount_amount"
                                value="{{ old('discount_amount', $product->discount_amount) }}" min="0" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3F3142] focus:border-transparent">
                        </div>

                        <div>
                            <label for="is_on_sale" class="block text-sm font-medium mb-2">On Sale *</label>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_on_sale" id="is_on_sale" value="1"
                                    {{ old('is_on_sale', $product->is_on_sale) ? 'checked' : '' }} class="sr-only peer">

                                <div
                                    class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-[#3F3142]/20 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#3F3142]">
                                </div>

                                <span class="ms-3 text-sm font-medium text-gray-900">Aktifkan Diskon</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        @if ($product->image_url)
                            <div class="mb-2">
                                ...
                                <img src="{{ $product->image_path }}" class="w-full h-auto object-cover rounded border"
                                    alt="">
                            </div>
                        @endif
