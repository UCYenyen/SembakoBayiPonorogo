                            ...
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex-1">
                                    @if ($address->is_default)
                                        <span
                                            class="inline-block px-3 py-1 bg-[#3F3142] text-white text-xs font-semibold rounded-full mb-2">
                                            Alamat Default
                                        </span>
                                    @endif
                                    <h2 class="text-xl font-bold mb-2">{{ $address->name }}</h2>
                                    <p class="text-gray-700 mb-1">{{ $address->extra_detail }}</p>
                                    <p class="text-gray-700 mb-1">
                                        {{ $address->subdistrict_name }}, {{ $address->district_name }},
                                        {{ $address->city_name }}, {{ $address->province_name }}
                                    </p>
                                    <p class="text-gray-700">Kode Pos: {{ $address->postal_code }}</p>
                                </div>
                            </div>
