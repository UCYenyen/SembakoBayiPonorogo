...
@section('content')
    <main class="bg-[#FFF3F3] text-[#3F3142] min-h-screen py-8">
        <div class="w-[80%] mx-auto">
            ...
            <form action="{{ route('user.testimonies.store') }}" method="POST" class="bg-white shadow-lg rounded-lg px-6 py-4 mb-8" enctype="multipart/form-data" id="testimony-form">
                @csrf
                ...
                <button type="submit" id="submit-btn" class="bg-[#3F3142] text-white px-6 py-2 rounded-lg hover:bg-[#5C4B5E] font-semibold transition-colors">
                    Submit Ulasan
                </button>
            </form>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updatePreview() {
                ...
                filesArray.forEach((file, index) => {
                    ...
                    `;
                    deleteBtn.className = 'absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 transition-colors opacity-0 group-hover:opacity-100';
