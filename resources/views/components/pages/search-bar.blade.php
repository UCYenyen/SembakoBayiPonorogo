    ...
    <form action="{{ url($route) }}" method="GET" class="flex gap-2 w-full">
        <div class="relative flex-1">
            ...
            <div x-show="loading" class="absolute right-3 top-1/2 -translate-y-1/2">
                <svg class="animate-spin h-5 w-5 text-[#3F3142]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                       ...
                       class="flex items-center gap-3 p-3 hover:bg-gray-50 transition-colors border-b last:border-b-0">
                        ...
                        <div class="flex-shrink-0">
                            <span 
                                ...
                                :class="product.stocks > 10 ? 'bg-green-100 text-green-800' : (product.stocks > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800')"
            ...
            {{-- No Results Message --}}
            <div 
                x-show="open && query.length > 0 && results.length === 0 && !loading"
                x-transition
        ...
        <button 
            ...
            class="px-6 py-2 bg-[#3F3142] hover:bg-[#5C4B5E] text-white rounded-lg transition-colors duration-200 text-sm md:text-base whitespace-nowrap">
